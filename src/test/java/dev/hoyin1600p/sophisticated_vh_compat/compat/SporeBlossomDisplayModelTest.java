...
class SporeBlossomDisplayModelTest {
    ...
    void vanillaAttachmentIsFlushAndAllPetalsProjectOutward() throws Exception {
        BakedModel model = SporeBlossomDisplayModel.wrap(new ItemStack(Items.SPORE_BLOSSOM), originalModel());
        Matrix4f transform = TransformationHelper.toTransformation(model.getTransforms().fixed).getMatrix();
        try (var stream = getClass().getResourceAsStream("/assets/minecraft/models/block/spore_blossom.json")) {
            assertNotNull(stream, "Validate against the actual Minecraft model geometry");
            var elements = JsonParser.parseReader(new InputStreamReader(stream, StandardCharsets.UTF_8))
                    .getAsJsonObject().getAsJsonArray("elements");
            for (int elementIndex = 0; elementIndex < elements.size(); elementIndex++) {
                var element = elements.get(elementIndex).getAsJsonObject();
                ...
                for (int corner = 0; corner < 8; corner++) {
                    ...
                    if (element.has("rotation")) {
                        ...
                        var xyz = rotation.getAsJsonArray("origin");
    ...
    void leavesOtherItemsAndTheSharedModelUnchanged() {
        BakedModel original = originalModel();
